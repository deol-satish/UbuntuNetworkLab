 ...
 */
                    ...
                    struct sk_buff **to_free)
 {
     ...
     if (skb_in_l_queue(skb)) {
        pr_info("skb_in_l_queue(skb) : so in l_queueu \n");
                  ...
                  struct sk_buff **to_free)
 {
     ...
     u64 skb_qdelay = 0;
     now = ktime_get_ns();
     skb_qdelay = skb_sojourn_time(skb, now);

     pr_info("DUALPI2: dualpi2_qdisc_enqueue - skb_qdelay: %llu\n",(unsigned long long)skb_qdelay);
 ...
 static u32 calculate_probability(struct Qdisc *sch)
 {
     ...
     pr_info("DUALPI2: calculate_probability - qdelay_c: %llu, qdelay_l: %llu\n",(unsigned long long)qdelay_c,(unsigned long long)qdelay_l);
